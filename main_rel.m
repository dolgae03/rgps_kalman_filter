...
%% 시뮬레이션 데이터 추출
num_iterations = 5000; % 시간 단계 수
convergence_idx = 4800;
...
%% Kalman Filter 정의
kalman_filter = TC_REL_KF(init_x, P);
...
for k = 1:num_iterations
    ...
    %% Correction 단계
    R = 0.001 .* eye(size(measurements, 1));
    ...
    current_state = kalman_filter.state;

    % 추정된 상태 저장
    ls_position(:, k) = ref_pos2 - ref_pos;
    estimated_states(:, k) = current_state;
end
...
% LS Position 에러 계산
ls_error_x = true_position(1, convergence_idx:end) - ls_position_rel(1, convergence_idx:end);
ls_error_y = true_position(2, convergence_idx:end) - ls_position_rel(2, convergence_idx:end);
ls_error_z = true_position(3, convergence_idx:end) - ls_position_rel(3, convergence_idx:end);
